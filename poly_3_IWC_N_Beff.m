...
B_crv = 1:0.0005:2 ;
p = polyfit(Beff(isnan(N_IWC)==0),N_IWC(isnan(N_IWC)==0),3);
f = polyval(p,B_crv);

plot(B_crv,f,'-k','LineWidth',2)
  
[ffit,gof2] = fit(Beff(isnan(N_IWC)==0),N_IWC(isnan(N_IWC)==0),'poly3') ;

cor=sprintf('R^2=%g',gof2.rsquare);
cor2=sprintf('RMSE=%g',gof2.rmse);
equation=sprintf('y=%gx^3+%gx^2%gx+%g',p(1),p(2),p(3),p(4));
str = {equation , cor, cor2};
rrr=annotation('textbox', [.23 .33, .1, .1], 'String', str);
set(rrr,'Fontsize',13)
