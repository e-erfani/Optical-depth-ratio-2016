...
plot_control
  ...
  purple = [0.5,0,0.5] ;
    ...
    fnames = dir('Beff2*.txt') ;
        ...
        fig_name = 'power_inverese_Qeff_Beff';
        fig_dum = figure(1);
...
B_crv = 1:0.0005:1.7 ;
p = polyfit(log(Beff(isnan(inv_Qeff)==0)),log(inv_Qeff(isnan(inv_Qeff)==0)),1);
%f = polyval(exp(p),exp(B_crv));
f = polyval(p,log(B_crv));

plot(B_crv,exp(f),'-k','LineWidth',2)
%plot(B_crv,f,'-k','LineWidth',2)
[ffit,gof2] = fit(log(Beff(isnan(inv_Qeff)==0)),log(inv_Qeff(isnan(inv_Qeff)==0)),'poly1') ;
...
ylabel('2/Q_a_b_s(12\mum)eff (unitless)','fontSize',h_axis+6);
box on
%ylim([1E5 1E9])
xlim([1 1.6])
