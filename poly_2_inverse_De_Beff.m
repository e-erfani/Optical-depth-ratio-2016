...
plot_control
  ...
  purple = [0.5,0,0.5] ;
    ...
    fnames = dir('Beff2*.txt') ;
        ...
        fig_name = '2_poly_inverese_De_Beff';
        fig_dum = figure(1);
...
B_crv = 1:0.0005:1.7 ;
p = polyfit(Beff(isnan(inv_De)==0),inv_De(isnan(inv_De)==0),2);
f = polyval(p,B_crv);

plot(B_crv,f,'-k','LineWidth',2)
  
[ffit,gof2] = fit(Beff(isnan(inv_De)==0),inv_De(isnan(inv_De)==0),'poly2') ;

cor=sprintf('R^2=%g',gof2.rsquare);
cor2=sprintf('RMSE=%g',gof2.rmse);
equation=sprintf('y=%gx^2+%gx%g',p(1),p(2),p(3));
str = {equation , cor, cor2};
rrr=annotation('textbox', [.467 .33, .1, .1], 'String', str);
set(rrr,'Fontsize',15)
