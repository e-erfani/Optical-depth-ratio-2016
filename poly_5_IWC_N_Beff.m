...
B_crv = 1:0.05:1.7 ;
p = polyfit(Beff(isnan(N_IWC)==0),N_IWC(isnan(N_IWC)==0),5);
f = polyval(p,B_crv);

% crvft = p(1).*B_crv.^2 + p(2).*B_crv + p(3) ;
semilogy(B_crv,f,'-k','LineWidth',2)
  
[ffit,gof2] = fit(Beff(isnan(N_IWC)==0),N_IWC(isnan(N_IWC)==0),'poly5') ;

cor=sprintf('R^2=%g',gof2.rsquare);
cor2=sprintf('RMSE=%g',gof2.rmse);
equation=sprintf('y=%gx^5+%gx^4+%gx^3+%gx^2+%gx+%g',p(1),p(2),p(3),p(4),p(5),p(6));
